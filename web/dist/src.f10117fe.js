...
function (_super) {
  ...
  UserForm.prototype.eventMap = function () {
    return {
      ...
      'click:.save-model': this.onModelSave
    };
  };
...
function () {
  ...
  Model.prototype.save = function () {
    ...
    var temp = this.attributes.getAll();
    this.sync.save(temp).then(function (res) {
      _this.trigger('save');
    }).catch(function () {
      _this.trigger('error');
    });
...
function () {
  function Eventing() {
    ...
    this.events = {}; //eventing system

    this.on = function (eventName, callback) {
      var handlers = _this.events[eventName] || []; //if there are is an existing array associated with the event name set handlers to that array or else initalize handlers to an empty array

      handlers.push(callback); //add a new function to this array

      _this.events[eventName] = handlers; //set the key in the events object to the array of callbacks
    };
